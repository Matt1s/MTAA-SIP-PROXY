...
# Renaming each occurence to socketserver instead of SocketServer
import socketserver 
import re
import time
import logging
...
class UDPHandler(socketserver.BaseRequestHandler):   
    ...
    def processRegister(self):
        ...
        # uncommented due to IP address restrictions 
        ...
        logging.info("ZDROJ: %s - SIP Number: %s" % (fromm,contact))
    ...
    def processInvite(self):
        ...
        if len(destination) > 0:
            logging.info("CIEĽ: %s" % destination)
    ...
    def processAck(self):
        ...
        #added next line for better logging
        source = self.getOrigin()
        if len(destination) > 0:
            logging.info("ACK - Hovor prebieha zo ZDROJA: %s k CIELU: %s" % (source,destination))
    ...
    def processNonInvite(self):
        ...
        origin = self.getOrigin()
        if len(origin) == 0 or not origin in registrar:
            self.sendResponse("400 Bad Request - nespravna poziadavka")
            return
        destination = self.getDestination()
        if len(destination) > 0:
            logging.info("CIEL: %s" % destination)
            if destination in registrar and self.checkValidity(destination):
                socket,claddr = self.getSocketInfo(destination)
                #self.changeRequestUri()
                self.data = self.addTopVia()
